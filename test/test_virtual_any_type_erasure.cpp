...
// -----------------------------------------------------------------------------
// virtual_any_ref: the non-owning counterpart

MAKE_CLASSES();

BOOST_OPENMETHOD(name, (virtual_any_ref<const erased>), std::string);
...
using name_method =
    BOOST_OPENMETHOD_TYPE(name, (virtual_any_ref<const erased>), std::string);
...
BOOST_OPENMETHOD(bump, (virtual_any_ref<erased>), std::string);

using bump_method =
    BOOST_OPENMETHOD_TYPE(bump, (virtual_any_ref<erased>), std::string);
...
BOOST_AUTO_TEST_CASE(virtual_any_ref_by_value) {
    initialize(trace());

    // from an `any`: the v-table pointer is looked up
    const erased spot_any(Dog{"Spot"});
    virtual_any_ref<const erased> spot = spot_any;
    BOOST_TEST(spot.vptr() == default_registry::static_vptr<Dog>);
    ...
    virtual_erased rex = Dog{"Rex"};
    virtual_any_ref<const erased> rex_cref = rex;
    ...
    virtual_any_ref<erased> rex_ref = rex;
    ...
    virtual_any_ref<const erased> const_rex = rex_ref;
    BOOST_TEST(const_rex.vptr() == rex_ref.vptr());
}
} // namespace BOOST_OPENMETHOD_GENSYM
