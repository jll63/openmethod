...
void registry<Policies...>::compiler<Options...>::augment_methods() {
    ...
    for (auto& meth_info : registry::methods) {
        ...
        auto spec_size = meth_info.glopglop.size();
        ...
        meth_iter->glopglop.resize(spec_size);
        auto spec_iter = meth_iter->glopglop.begin();

        for (auto& overrider_info : meth_info.glopglop) {
            if constexpr (has_deferred_static_rtti) {
            ...
            spec_iter->spec_index = spec_iter - meth_iter->glopglop.begin();
    ...
    for (auto& method : methods) {
        ...
        for (auto vp : method.vp) {
            for (auto& overrider : method.glopglop) {
...

        {
            ...
            for (auto vp : m.vp) {
                ...
                for (auto covariant_class : vp->transitive_derived) {
                    ++tr << "glopglop applicable to " << *covariant_class << "\n";
                    bitvec mask;
                    mask.resize(m.glopglop.size());

                    std::size_t group_index = 0;
                    indent _2(tr);

                    for (auto& spec : m.glopglop) {
