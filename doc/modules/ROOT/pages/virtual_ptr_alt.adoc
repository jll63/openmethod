...
`virtual_` parameter, it looks for a cpp:boost_openmethod_vptr[] function that
can be called with the virtual argument (passed by const reference) and a
...
reference or a pointer to an object. There, though, it is an optimization, not a
way of widening dispatch: those constructors and assignment operators require a
polymorphic class in any case. Only a `virtual_` parameter can dispatch on a
class that is not polymorphic.
...
variable, and a cpp:boost_openmethod_vptr[] friend function that returns its
value -
