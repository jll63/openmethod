...
cpp:virtual_std_any[] (see below) registers its type as well. A type that is
...
Building from a value, or `emplace`, also _registers_ the type, like naming it
...
A `virtual_std_any` cannot itself be wrapped in a cpp:virtual_ptr[]: it is
...
time, cpp:final_virtual_ptr[] included. Pass it by reference instead.

#### `boost::any`

`boost::any` is supported as well, by
`<boost/openmethod/interop/boost_any.hpp>`, with cpp:virtual_boost_any[] - the
exact counterpart of cpp:virtual_std_any[]. The two root classes are distinct,
