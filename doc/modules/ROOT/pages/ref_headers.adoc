...
[#virtual_any_ptr]
### link:{headers-url}/boost/openmethod/interop/virtual_any_ptr.hpp[<boost/openmethod/interop/virtual_any_ptr.hpp>]

Provides `virtual_any_ptr`, the non-owning counterpart of `virtual_any`: a wide
pointer to an `any` that lives elsewhere, combining its address with a pointer
to the v-table for the contained value.
