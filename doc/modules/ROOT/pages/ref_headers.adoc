...
[#boost_type_erasure]
### link:{{BASE_URL}}/include/boost/openmethod/interop/boost_type_erasure.hpp[<boost/openmethod/interop/boost_type_erasure.hpp>]
...
up only once, at construction. In addition, the header deletes the
`final_virtual_ptr` overloads for `boost::type_erasure::any`, which would
otherwise silently use the v-table of the `any` root class instead of the bound
value's.
