...
OpenMethod interacts well with dynamic loading of libraries if the platform and
toolchain make global variables truly global.
...
After loading a dynamic library, the program must call `initialize` to rebuild
the dispatch tables. If `virtual_ptr`{empty}s already exist
...
Now back to `main`. We need to call `initialize` before using any
