...
That registry has methods of its own. `value` evaluates a node, and takes
nothing but nodes, so it is an ordinary single-registry method:

[source,c++]
----
include::example$mixed_registries.cpp[tag=value]
----
...
that registry, beside the slots of the methods that registry holds itself, and
the method's registry fills it. The parameter's registry must therefore be
initialized first:
...
Calls work as usual - `render` dispatching in both registries at once, `value`
in `node_registry` alone:
...
This prints `1 2 + 3 + = 6`, then `((1 + 2) + 3) = 6`.
...
Registries whose `rtti` policy derives from `deferred_static_rtti` mix with
those whose policy does not: the type ids of a parameter are resolved when its
own registry resolves them.
