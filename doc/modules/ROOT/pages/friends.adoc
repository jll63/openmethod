...
`BOOST_OPENMETHOD_MEM` instead of `BOOST_OPENMETHOD`:
...
`BOOST_OPENMETHOD_OVERRIDE_MEM` instead of `BOOST_OPENMETHOD_OVERRIDE`:
...
`ID` names the method being overridden - here `Zoo::poke` - and may equally
name a free method declared with `BOOST_OPENMETHOD`.
...
through the core API instead, via `BOOST_OPENMETHOD_OVERRIDER_MEM`, naming
*itself* (its own `(Class, ID, PARAMETERS, RETURN)`), not the overrider it
calls.
