...
cpp:register_classes_opts[] options with `|`; and reflections of registries to
register the classes in - cpp:default_registry[] when none is named. A
...
program never heard of anyway. The cpp:register_classes_opts::scan_std[] and
cpp:register_classes_opts::scan_boost[] options bring them back in; a namespace
*listed* explicitly is always scanned. cpp:register_classes_opts::no_recurse[]
restricts the scan to the members declared directly in the listed namespaces.
