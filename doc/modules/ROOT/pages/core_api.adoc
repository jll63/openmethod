...
for that: xref:reference:BOOST_OPENMETHOD_ID.adoc[BOOST_OPENMETHOD_ID]. Let's use it:
...
high-level macros like xref:reference:BOOST_OPENMETHOD.adoc[BOOST_OPENMETHOD]. This makes
...
xref:reference:BOOST_OPENMETHOD_REGISTER.adoc[BOOST_OPENMETHOD_REGISTER]. It takes a
...
With a compiler that supports C++26 reflection, cpp:register_classes[] replaces
that list. It scans the global namespace - or the ones it is given - for the
registry's methods, and registers the classes they dispatch on along with
everything in those namespaces that derives from them:
...
Macro xref:reference:BOOST_OPENMETHOD_TYPE.adoc[BOOST_OPENMETHOD_TYPE] takes the same
